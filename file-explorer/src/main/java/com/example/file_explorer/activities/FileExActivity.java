...
public class FileExActivity extends AppCompatThemeActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, FileExActivity.class);
    ...
    public class SectionsPagerAdapter extends FragmentPagerAdapter {
        ...
        public void update() {
            ...
            FilesTabView leftTab = new FilesTabView(FileExActivity.this, tabLayout);
            leftTab.attachLayout(left);
            TabLayout.Tab right = tabLayout.getTabAt(1);
            FilesTabView rightTab = new FilesTabView(FileExActivity.this, tabLayout);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = ActivityFileExplorerBinding.inflate(getLayoutInflater());
        ...
        ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(this, drawer, mBinding.appBarMain.toolbar, R.string.navigation_drawer_open, R.string.navigation_drawer_close);
        drawer.addDrawerListener(toggle);
        toggle.syncState();
        tabLayout = mBinding.appBarMain.tabs;

        final FloatingActionsMenu fab = mBinding.appBarMain.fab;
        FloatingActionButton fabFolder = mBinding.appBarMain.fabCreateFolder;
        fabFolder.setOnClickListener(v -> {
            final OpenFileDialog.EditTextDialog edit = new OpenFileDialog.EditTextDialog(FileExActivity.this);
            edit.setTitle(R.string.create_folder);
            edit.setPositiveButton(android.R.string.ok, (dialog, which) -> {
                String s = edit.getText();
                final FilesFragment f = mSectionsPagerAdapter.getActiveFragment();
                try {
                    final Uri uri = storage.mkdir(f.getUri(), s);
                    if (uri == null) throw new RuntimeException("unable to create " + s);
                    f.reload();
                    f.highlight(uri);
                } catch (RuntimeException e) {
                    Log.d(TAG, "create folder", e);
                    Toast.makeText(FileExActivity.this, R.string.not_permitted, Toast.LENGTH_SHORT).show();
                } finally {
                    storage.closeSu();
                }
            });
        ...
        FloatingActionButton fabFile = mBinding.appBarMain.fabCreateFile;
        fabFile.setOnClickListener(v -> {
            ...
            edit.setPositiveButton(android.R.string.ok, (dialog, which) -> {
                ...
                try {
                    ...
                    if (uri == null) throw new RuntimeException("unable to create file");
                    f.reload();
                    f.highlight(uri);
                } catch (RuntimeException e) {
        ...
        if (a.equals(Intent.ACTION_VIEW)) {
            try {
                ...
                if (Build.VERSION.SDK_INT >= 21 && intent.getType().equals(DocumentsContract.Root.MIME_TYPE_ITEM)) {
                    ...
                    try {
                        Storage.takePersistableUriPermission(this, u, Storage.SAF_RW); // refresh perms
                        ...
                        builder.setNeutralButton(R.string.add_bookmark, new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                choicer.setStorageAccessFramework(FileExActivity.this, RESULT_ADDBOOKMARK);
                                choicer.show(old);
                            }
                        });
...
//        collapseListener.addItem(search);
        ...
        searchView.setOnCloseButtonListener(new SearchView.OnCloseButtonListener() {
            ...
            public void onClosed() {
                if (FileExActivity.this.search != null) {
                    if (FileExActivity.this.search.isActive()) {
                        FileExActivity.this.search.stop();
                    }
                }
                oldSearch = "";
            }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_addstorage) {
            ...
            choicer.setPermissionsDialog(this, Storage.PERMISSIONS_RO, RESULT_ADDBOOKMARK);
            Uri old = mSectionsPagerAdapter.getActiveFragment().getUri();
            if (!old.getScheme().equals(ContentResolver.SCHEME_FILE)) old = null;
            choicer.show(old);
            return true;
        }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (intent != null) {
            ...
            if (a.equals(ADD_BOOKMARK)) {
                ...
                if (Build.VERSION.SDK_INT < 21) m.removeItem(R.id.action_addsaf);
                if (m.size() == 1) {
                    onOptionsItemSelected(m.getItem(0));
                } else {
                    menu.setOnMenuItemClickListener(item1 -> onOptionsItemSelected(item1));
                    menu.show();
                }
                return true;
            }
    ...
    public String getDrawerName(Uri u) {
        ...
        if (s.equals(ContentResolver.SCHEME_FILE)) {
            ...
            if (f.equals(Environment.getExternalStorageDirectory())) return f.getPath();
            ...
            b.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    builder.setPositiveButton(android.R.string.ok, (dialog, which) -> {
                        EmptyTrashBuilder delete = new EmptyTrashBuilder(FileExActivity.this);
                        delete.show();
                    });
                    builder.setNegativeButton(android.R.string.cancel, (dialog, which) -> {
                    });
    ...
    public void supportInvalidateOptionsMenu() {
        if (search != null) return; // prevent update options, we need SearchView visible
    ...
    public void searchOpen(String q) {
        ...
        View v = mBinding.appBarMain.searchContainer;
